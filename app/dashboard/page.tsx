...
import { 
  ...
  Gamepad2, Quote, Flame, Star, BrainCircuit
} from "lucide-react";
...
export default function MassiveDashboard() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        if (!userData.grade || !userData.region) {
        ...
        // 2. Setup Daily Wisdom (Pseudo-random based on day)
        const quotes = [
        ...
        // 5. Fetch Exam Countdown
        const examSnap = await getDoc(doc(db, "examm", "examm"));
        if (examSnap.exists()) setExamDays(examSnap.data().days || 0);

        // 6. Simulate Recent Content (In a real app, query recent exams/lessons by createdAt)
        setRecentContent([
          { id: "1", title: "Mathematics Past Paper 2023", type: "exam", subjectId: fetchedSubjects[0]?.id || "" },
          { id: "2", title: "Introduction to Algebra", type: "lesson", subjectId: fetchedSubjects[1]?.id || "" },
        ]);
        ...
        if (err.message?.includes("requires an index")) {
          ...
          setError("Database indexing required. Please generate the index to continue.");
        } else {
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-28 font-sans selection:bg-blue-100">
      ...
      {/* =========================================
          SECTION 1: THE SMART HEADER (Mobile Fixed)
          ========================================= */}
      <header className="bg-gradient-to-br from-blue-700 via-blue-600 to-indigo-700 pt-8 pb-16 md:pt-14 md:pb-28 px-4 sm:px-6 lg:px-8 rounded-b-[32px] md:rounded-b-[48px] shadow-2xl relative overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 -ml-20 -mb-20 w-80 h-80 bg-blue-400 opacity-20 rounded-full blur-3xl mix-blend-overlay"></div>
        ...
        <div className="max-w-6xl mx-auto flex justify-between items-center relative z-10">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-2xl md:text-4xl font-extrabold text-white tracking-tight mb-0.5 md:mb-1 flex items-center">
                Welcome back, {userProfile?.displayName.split(" ")[0]} <Sparkles className="w-5 h-5 ml-2 text-yellow-300" />
              </h1>
              <div className="flex items-center space-x-2 text-blue-100 font-medium text-xs md:text-sm">
                <span className="bg-white/20 px-2.5 py-0.5 rounded-full backdrop-blur-sm border border-white/10">{userProfile?.grade}</span>
                <span className="bg-white/20 px-2.5 py-0.5 rounded-full backdrop-blur-sm border border-white/10">{userProfile?.region}</span>
              </div>
            </div>
          </div>
          
          {/* Pro Badge (Desktop Only to save mobile space) */}
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 -mt-10 md:-mt-16 space-y-8 md:space-y-12 relative z-20">
        ...
        <div className="flex overflow-x-auto pb-6 snap-x snap-mandatory hide-scrollbar space-x-4 md:space-x-6">
          ...
          {/* Social Communities */}
          <QuickActionCard 
            href="https://portfolio.mubarikosman.com/telegram" 
            icon={<Send className="w-6 h-6" />} 
            title="Telegram Hub" 
            desc="Get instant updates and news."
            colors="from-sky-400 to-blue-500" 
          />
          <QuickActionCard 
            href="https://portfolio.mubarikosman.com/whatsapp" 
            icon={<MessageCircle className="w-6 h-6" />} 
            title="WhatsApp Group" 
            desc="Chat with peers and mentors."
            colors="from-emerald-400 to-teal-500" 
          />
        </div>

        {/* SECTION 3: DAILY WISDOM (Glassmorphism Panel) */}
        <section className="bg-white rounded-[32px] p-1 shadow-sm border border-slate-200/60 overflow-hidden">
          <div className="bg-gradient-to-r from-slate-50 to-blue-50/30 rounded-[28px] p-6 md:p-8 flex items-center relative overflow-hidden">
            <Quote className="absolute -left-4 -top-4 w-32 h-32 text-blue-500/5 rotate-12" />
            <div className="relative z-10 flex-1 pl-4 md:pl-8 border-l-4 border-blue-500">
              <h3 className="text-xl md:text-2xl font-bold text-slate-800 leading-snug mb-3">"{wisdomQuote.text}"</h3>
        ...
        {/* SECTION 4: RECENT CONTENT (Continue Learning) */}
        ...
        {/* SECTION 5: GAMIFICATION & QUIZZES */}
        <section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {quizzes.length > 0 ? quizzes.map((quiz) => (
              ...
              <div className="col-span-full bg-white p-8 rounded-[24px] text-center border border-slate-200">
                ...
                <p className="text-slate-500 font-medium">New quiz challenges unlocking soon!</p>
              </div>
            )}
          </div>
        </section>

        {/* SECTION 6: CORE SUBJECTS GRID */}
        <section>
          <SectionHeader title="Your Curriculum" subtitle={`${subjects.length} Subjects for ${userProfile?.grade}`} />
          {subjects.length === 0 ? (
             <div className="bg-white rounded-[32px] p-12 text-center border border-slate-200 shadow-sm">
             <BookOpen className="w-16 h-16 text-slate-200 mx-auto mb-4" />
             <h3 className="text-xl font-bold text-slate-700">Curriculum Updating</h3>
             <p className="text-slate-500 mt-2">Content for your specific region is being synced.</p>
           </div>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
              {subjects.map((subject) => (
                <Link href={`/subjects/${subject.id}`} key={subject.id} className="bg-white rounded-[28px] p-4 shadow-sm border border-slate-200 hover:shadow-xl hover:border-blue-300 transition-all duration-300 group flex flex-col">
                  ...
                  <div className="flex items-center text-xs text-slate-400 font-bold uppercase tracking-wider mt-auto">
                    <span>Explore</span>
