...
export default function QuizAuto({ onParseComplete }: QuizAutoProps) {
  ...
  const handleReview = (e: React.FormEvent) => {
    e.preventDefault();
    if (!title || !subjectId || !grade || !region || parsedData.length === 0) {
      return alert("Fill all fields and upload a valid JSON.");
    }
    
    // Pass everything to the Manual Editor for review!
    onParseComplete({
      title,
      subjectId,
      grade,
  ...
  return (
    <form onSubmit={handleReview} className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <select required value={subjectId} onChange={(e) => setSubjectId(e.target.value)} className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-emerald-500 outline-none font-bold text-slate-800">
          ...
          <div>
            ...
            <select required value={grade} onChange={(e) => setGrade(e.target.value)} className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-emerald-500 outline-none font-bold text-slate-800">
          ...
          <div>
            ...
            <select required value={region} onChange={(e) => setRegion(e.target.value)} className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-emerald-500 outline-none font-bold text-slate-800">
      ...
      <div className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-200 text-center">
        ...
        {!parsedData.length ? (
          <div onClick={() => fileInputRef.current?.click()} className="cursor-pointer py-16 border-4 border-dashed border-slate-200 rounded-[24px] hover:bg-emerald-50 hover:border-emerald-300 transition-all flex flex-col items-center justify-center">
            ...
            <p className="text-slate-500 font-bold max-w-sm">File must contain an array of questions.</p>
          </div>
        ) : (
          <div className="bg-emerald-50 border border-emerald-200 p-8 rounded-[24px] flex items-center justify-between">
            <div className="flex items-center text-left">
              ...
              <div>
                ...
                <p className="text-emerald-700 font-bold text-lg"><CheckCircle2 className="w-5 h-5 inline mr-1 mb-1"/> Found {parsedData.length} questions ready for review.</p>
              </div>
            </div>
